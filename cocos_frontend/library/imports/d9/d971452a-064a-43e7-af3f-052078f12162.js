"use strict";
cc._RF.push(module, 'd9714UqBkpD568/BSB48SFi', 'TipPanel');
// Script/ui/TipPanel.ts

"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
var BWUnitManager_1 = require("../BWUnitManager");
var _a = cc._decorator, ccclass = _a.ccclass, property = _a.property;
var TipPanel = /** @class */ (function (_super) {
    __extends(TipPanel, _super);
    function TipPanel() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.labelpid = null;
        return _this;
    }
    TipPanel.prototype.onLoad = function () {
        cc.log('wangbin onLoa1 ');
        //EventManager.Instance.AddEvent(ECMDID.LOGIN, this, this.testCallback);
        //EventManager.Instance.RemoveEvent(ECMDID.LOGIN, this, this.start)
        //EventManager.Instance.FireEvent(ECMDID.LOGIN, 'wangbin');
        this.labelpid = this.node.getChildByName('labelpid').getComponent(cc.Label);
    };
    TipPanel.prototype.onDestroy = function () {
        //EventManager.Instance.RemoveEvent(ECMDID.LOGIN, this, this.testCallback)
    };
    TipPanel.prototype.start = function () {
    };
    TipPanel.prototype.update = function (dt) {
        //cc.log("gameapp=dt="+dt);
        BWUnitManager_1.default.Instance.update(dt);
    };
    TipPanel.prototype.lateUpdate = function () {
    };
    TipPanel.prototype.onTipEnd = function () {
        this.node.destroy();
        //this.node.removeFromParent();
    }; ///
    TipPanel.prototype.showTip = function (strTip) {
        var _this = this;
        cc.log('showTip=' + strTip);
        this.labelpid.string = strTip;
        // 本节点向上移动300像素，然后淡出消失（用 Tween，避免 cc.Action deprecated）
        cc.Tween.stopAllByTarget(this.node);
        this.node.opacity = 255;
        cc.tween(this.node)
            .by(10, { y: 900 })
            .to(1, { opacity: 0 })
            .call(function () { return _this.onTipEnd(); })
            .start();
    };
    __decorate([
        property(cc.Label)
    ], TipPanel.prototype, "labelpid", void 0);
    TipPanel = __decorate([
        ccclass
    ], TipPanel);
    return TipPanel;
}(cc.Component));
exports.default = TipPanel;

cc._RF.pop();